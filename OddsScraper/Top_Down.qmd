...
over_markets <-
  ...
  mutate(edge = (pinnacle_over_implied_prob - over_implied_prob) * 100) |>
  mutate(edge = round(edge, 2)) |>
  relocate(edge, .after = agency) |>
  arrange(desc(edge))

{
  out <- over_markets |>
    filter(edge >= 2) |>
    mutate(across(matches("implied_prob$"), ~ round(.x, 3)))
  knitr::kable(out, escape = FALSE) |>
    kable_styling() |>
    column_spec(which(names(out) == "edge"), bold = TRUE)
}
```
...
under_markets <-
  ...
  mutate(edge = (pinnacle_under_implied_prob - under_implied_prob) * 100) |>
  mutate(edge = round(edge, 2)) |>
  relocate(edge, .after = agency) |>
  arrange(desc(edge))
...
over_markets <-
  ...
  mutate(edge = (sportsbet_over_implied_prob - over_implied_prob) * 100) |>
  mutate(edge = round(edge, 2)) |>
  relocate(edge, .after = agency) |>
  arrange(desc(edge))
...
under_markets <-
  ...
  mutate(edge = (sportsbet_under_implied_prob - under_implied_prob) * 100) |>
  mutate(edge = round(edge, 2)) |>
  relocate(edge, .after = agency) |>
  arrange(desc(edge))
